...
typedef boost::error_info<struct tag_newgenememory, std::pair<int, int>> newgene_memory_error_tag;
...
class pool_allocator
{
  public:
    ...
    struct rebind
    { //
      typedef pool_allocator<U, UserAllocator, Tag, Mutex, NextSize, MaxSize> other;
    };
  ...
  public:
    ...
    { /*! Results in default construction of the underlying singleton_pool IFF an
       ...
       */
      singleton_pool<Tag, sizeof(T), UserAllocator, Mutex,
    ...
    template <typename U>
    pool_allocator(const pool_allocator<U, UserAllocator, Tag, Mutex, NextSize, MaxSize> &)
    ...
    static pointer allocate(const size_type n)
    {
      ...
      if ((ret == 0) && n)
		  BOOST_THROW_EXCEPTION(boost::enable_error_info(std::runtime_error("Bad allocation")) << newgene_memory_error_tag(std::make_pair(n, sizeof(T))));
      return ret;
    }
...
template<
    ...
    unsigned NextSize,
    unsigned MaxSize>
class pool_allocator<void, UserAllocator, Tag, Mutex, NextSize, MaxSize>
{
public:
...
class fast_pool_allocator
{
  public:
    ...
    struct rebind
    {
      typedef fast_pool_allocator<U, UserAllocator, Tag, Mutex, NextSize, MaxSize> other;
    };
  ...
  public:
    ...
    template <typename U>
    fast_pool_allocator(
        const fast_pool_allocator<U, UserAllocator, Tag, Mutex, NextSize, MaxSize> &)
    {
      //! Ensures construction of the underlying singleton_pool IFF an
      //! instance of this allocator is constructed during global
      //! initialization. See ticket #2359 for a complete explanation
    ...
    { //! Allocate memory.
      const pointer ret = static_cast<pointer>(
          (singleton_pool<Tag, sizeof(T),
              UserAllocator, Mutex, NextSize, MaxSize>::malloc)() );
      if (ret == 0)
		  BOOST_THROW_EXCEPTION(boost::enable_error_info(std::runtime_error("Bad allocation")) << newgene_memory_error_tag(std::make_pair(1, sizeof(T))));
      return ret;
    }
    static void deallocate(const pointer ptr, const size_type n)
    { //! Deallocate memory.
...
template<
    ...
    unsigned NextSize,
    unsigned MaxSize >
class fast_pool_allocator<void, UserAllocator, Tag, Mutex, NextSize, MaxSize>
{
public:
    typedef void*       pointer;
    typedef const void* const_pointer;
