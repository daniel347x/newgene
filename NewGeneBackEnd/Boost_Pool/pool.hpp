  ...
  if (ptr == 0)
  {
     if(next_size > 4)
     {
        ...
        ptr = (UserAllocator::malloc)(POD_size);
				if (ptr == 0)
				{
					BOOST_THROW_EXCEPTION(boost::enable_error_info(std::runtime_error("Bad allocation.  Total bytes requested follows.")) << newgene_internal_memory_error_tag(POD_size));
				}
     }
     if(ptr == 0)
        return 0;
