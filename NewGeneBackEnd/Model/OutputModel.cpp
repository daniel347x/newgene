...
void OutputModel::OutputGenerator::KadSamplerFillDataForChildGroups(KadSampler & allWeightings)
{
	...
	std::for_each(secondary_variable_groups_column_info.cbegin(), secondary_variable_groups_column_info.cend(), [&](ColumnsInTempView const & child_variable_group_raw_data_columns)
	{
		...
		// ********************************************************************************************************************************************************* //
		messager.AppendKadStatusText((boost::format("Create empty table for variable group \"%1%\")...") %
									  Table_VG_CATEGORY::GetVgDisplayTextShort(child_variable_groups_vector[current_child_vg_index].first)).str().c_str(), this);
...
void OutputModel::OutputGenerator::KadSamplerWriteResultsToFileOrScreen(KadSampler & allWeightings)
{
	...
	else
	{
		...
		// ************************************************************************************************************************ //
		auto loop_through_time_units = [&](TimeSlice const & timeSlice, fast_branch_output_row_set const & output_rows_for_this_full_time_slice,
