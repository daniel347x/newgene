...
CREATE TABLE [main].[DMU_SET_MEMBER] (
  ...
  [DMU_SET_MEMBER_FK_DMU_CATEGORY_UUID] char(36) NOT NULL ON CONFLICT Fail CONSTRAINT [DMU_SET_MEMBER_FK_CONSTRAINT] REFERENCES [DMU_CATEGORY]([DMU_CATEGORY_UUID]) ON DELETE CASCADE ON UPDATE CASCADE, 
...
CREATE TABLE [main].[VG_DATA_METADATA__DATETIME_COLUMNS] (
  ...
  [VG_DATA_FK_VG_CATEGORY_UUID] VARCHAR(128) NOT NULL ON CONFLICT FAIL CONSTRAINT [VG_DATA_METADATA__DATETIME_COLUMNS__FK] REFERENCES [VG_CATEGORY]([VG_CATEGORY_UUID]) ON DELETE CASCADE ON UPDATE CASCADE, 
...
CREATE TABLE [main].[VG_DATA_METADATA__PRIMARY_KEYS] (
  [VG_DATA_TABLE_NAME] CHAR(255) NOT NULL ON CONFLICT FAIL CONSTRAINT [VG_DATA_METADATA__PRIMARY_KEYS__FK] REFERENCES [VG_DATA_METADATA__DATETIME_COLUMNS]([VG_DATA_TABLE_NAME]) ON DELETE CASCADE ON UPDATE CASCADE, 
