...
/* Database [DefaultInputModel.newgene.in] */
...
CREATE TABLE [main].[VG_CATEGORY] (
  ...
  [VG_CATEGORY_STRING_CODE] VARCHAR(128) NOT NULL ON CONFLICT FAIL, 
...
CREATE TABLE [main].[VG_DATA_METADATA__DATETIME_COLUMNS] (
  [VG_DATA_TABLE_NAME] CHAR(255) NOT NULL ON CONFLICT FAIL REFERENCES [VG_DATA_METADATA__PRIMARY_KEYS]([VG_DATA_TABLE_NAME]) ON DELETE CASCADE ON UPDATE CASCADE, 
...
CREATE TABLE [main].[VG_DATA_METADATA__PRIMARY_KEYS] (
  [VG_DATA_TABLE_NAME] CHAR(256) NOT NULL ON CONFLICT FAIL, 
  [VG_DATA_TABLE_PRIMARY_KEY_COLUMN_NAME] CHAR(256) NOT NULL ON CONFLICT FAIL, 
  [VG_DATA_TABLE_FK_DMU_CATEGORY_CODE] CHAR(256) NOT NULL ON CONFLICT FAIL REFERENCES [DMU_CATEGORY]([DMU_CATEGORY_STRING_CODE]) ON DELETE CASCADE ON UPDATE CASCADE, 
  [VG_DATA_TABLE_PRIMARY_KEY_SEQUENCE_NUMBER] INT NOT NULL ON CONFLICT FAIL, 
  [VG_DATA_TABLE_PRIMARY_KEY_IS_NUMERIC] INT NOT NULL ON CONFLICT FAIL DEFAULT 0);
