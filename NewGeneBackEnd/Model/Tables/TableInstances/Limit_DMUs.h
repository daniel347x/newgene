...
class Table__Limit_DMUs__Elements : public Table<Table__Limit_DMUS__ELEMENTS, TABLE_INSTANCE_IDENTIFIER_CONTAINER_TYPE__MAP>
{
	...
	public:
		...
		bool Exists(sqlite3 * db, OutputModel & output_model_, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category, std::string const & dmu_member_uuid,
					bool const also_confirm_using_cache = true);
		...
		WidgetInstanceIdentifier AddDmuMember(sqlite3 * db, OutputModel & output_model_, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category,
