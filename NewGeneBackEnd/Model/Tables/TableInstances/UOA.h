...
class Table_UOA_Identifier : public Table<TABLE__UOA_IDENTIFIER, TABLE_INSTANCE_IDENTIFIER_CONTAINER_TYPE__VECTOR>
{
	...
	public:
		...
		WidgetInstanceIdentifiers RetrieveDMUCategories(sqlite3 const * db, InputModel * input_model_, NewGeneUUID const & uuid);
		...
		// Country=2, MID=1
		DMU_Counts RetrieveDMUCounts(sqlite3 const * db, InputModel * input_model_, NewGeneUUID const & uuid);
		...
		static std::string GetUoaCategoryDisplayText(WidgetInstanceIdentifier const & uoa_category, WidgetInstanceIdentifiers const & dmu_categories, bool const simple, bool const formatted);
