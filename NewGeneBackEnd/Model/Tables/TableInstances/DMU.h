...
class Table_DMU_Instance : public Table<TABLE__DMU_INSTANCE, TABLE_INSTANCE_IDENTIFIER_CONTAINER_TYPE__MAP>
{
	...
	public:
		...
		WidgetInstanceIdentifier CreateNewDmuMember(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category, std::string const & dmu_member_uuid,
				std::string const & dmu_member_code, std::string const & dmu_member_description);
		bool DeleteDmuMember(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_member);
		bool RefreshFromFile(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category, boost::filesystem::path const & dmu_refresh_file_pathname,
							 std::vector<std::string> const & dmu_column_labels, Messager & messager, bool const do_refresh_not_plain_insert);
