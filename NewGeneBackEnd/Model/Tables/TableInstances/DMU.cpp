...
bool Table_DMU_Identifier::CreateNewDMU(sqlite3 * db, InputModel & input_model_, std::string const & dmu, std::string const & dmu_description)
{
	...
	std::string flags;
	WidgetInstanceIdentifier DMU_category_identifier(new_uuid, new_dmu, dmu_description, 0, flags.c_str(), TIME_GRANULARITY__NONE, MakeNotes(std::string(), std::string(),
			std::string()));
	identifiers.push_back(DMU_category_identifier);
	Sort();
...
bool Table_DMU_Identifier::DeleteDMU(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier & dmu, DataChangeMessage & change_message)
{
	...
	identifiers.erase(std::remove_if(identifiers.begin(), identifiers.end(), std::bind(&WidgetInstanceIdentifier::IsEqual, std::placeholders::_1,
									 WidgetInstanceIdentifier::EQUALITY_CHECK_TYPE__UUID_PLUS_STRING_CODE, dmu)), identifiers.end());
...
void Table_DMU_Instance::Load(sqlite3 * db, InputModel * input_model_)
{
	...
	while ((step_result = sqlite3_step(stmt)) == SQLITE_ROW)
	{
		...
		if (uuid && fk_DMU_uuid)
		{
			...
			identifiers_map[fk_DMU_uuid].push_back(WidgetInstanceIdentifier(uuid, input_model_->t_dmu_category.getIdentifier(fk_DMU_uuid), code_string.c_str(), longhand_string.c_str(), 0,
												   flags_string.c_str(), TIME_GRANULARITY__NONE, MakeNotes(notes_string_1.c_str(), notes_string_2.c_str(), notes_string_3.c_str())));
		}
	}
...
bool Table_DMU_Instance::Exists(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category, std::string const & dmu_member_uuid,
								bool const also_confirm_using_cache)
{
...
WidgetInstanceIdentifier Table_DMU_Instance::CreateNewDmuMember(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category,
		std::string const & dmu_member_uuid, std::string const & dmu_member_code, std::string const & dmu_member_description)
{
...
bool Table_DMU_Instance::DeleteDmuMember(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_member)
{
			...
			std::for_each(input_model_.t_vgp_data_vector.begin(), input_model_.t_vgp_data_vector.end(), [&](std::unique_ptr<Table_VariableGroupData> & vg_instance_table)
			{
				if (vg_instance_table)
				{
					if (boost::iequals(*vg_to_delete.code, vg_instance_table->vg_category_string_code))
					{
						...
						std::vector<std::pair<WidgetInstanceIdentifier, std::vector<std::string>>> dmu_category_and_corresponding_column_names =
							input_model_.t_vgp_data_metadata__primary_keys.GetColumnNamesCorrespondingToPrimaryKeys(db, &input_model_, table_name_);
						std::for_each(dmu_category_and_corresponding_column_names.cbegin(),
									  dmu_category_and_corresponding_column_names.cend(), [&](std::pair<WidgetInstanceIdentifier, std::vector<std::string>> const & single_dmu_category_and_corresponding_column_names)
						{
	...
	// Remove from cache
	identifiers_map[*dmu_category.uuid].erase(std::remove_if(identifiers_map[*dmu_category.uuid].begin(),
			identifiers_map[*dmu_category.uuid].end(), [&](WidgetInstanceIdentifier & test_dmu_member)
	{
...
bool Table_DMU_Instance::RefreshFromFile(sqlite3 * db, InputModel & input_model_, WidgetInstanceIdentifier const & dmu_category,
		boost::filesystem::path const & dmu_refresh_file_pathname, std::vector<std::string> const & dmu_column_labels, Messager & messager, bool const do_refresh_not_plain_insert)
{
	...
	import_definition.format_qualifiers = ImportDefinition::FORMAT_QUALIFIERS__COMMA_DELIMITED | ImportDefinition::FORMAT_QUALIFIERS__STRINGS_ARE_EITHER_DOUBLEQUOTED_OR_SINGLEQUOTED |
	...
	mappings.push_back(std::make_shared<OneToOneFieldMapping>(std::make_pair(NameOrIndex(NameOrIndex::NAME, dmu_member_column_label_uuid), FIELD_TYPE_DMU_MEMBER_UUID),
					   std::make_pair(NameOrIndex(NameOrIndex::NAME, DMU_SET_MEMBER_UUID), FIELD_TYPE_DMU_MEMBER_UUID)));
	...
	if (!dmu_member_column_label_code.empty())
	{
		...
		mappings.push_back(std::make_shared<OneToOneFieldMapping>(std::make_pair(NameOrIndex(NameOrIndex::NAME, dmu_member_column_label_code), FIELD_TYPE_DMU_MEMBER_CODE),
						   std::make_pair(NameOrIndex(NameOrIndex::NAME, DMU_SET_MEMBER_STRING_CODE), FIELD_TYPE_DMU_MEMBER_CODE)));
	}
	...
	if (!dmu_member_column_label_description.empty())
	{
		...
		mappings.push_back(std::make_shared<OneToOneFieldMapping>(std::make_pair(NameOrIndex(NameOrIndex::NAME, dmu_member_column_label_description), FIELD_TYPE_DMU_MEMBER_DESCRIPTION),
						   std::make_pair(NameOrIndex(NameOrIndex::NAME, DMU_SET_MEMBER_STRING_LONGHAND), FIELD_TYPE_DMU_MEMBER_DESCRIPTION)));
	}
	...
	mappings.push_back(std::make_shared<HardCodedFieldMapping>(std::make_shared<Field<FIELD_TYPE_FK_TO_DMU_CATEGORY_UUID>>("DmuFK",
					   FieldValue<FIELD_TYPE_FK_TO_DMU_CATEGORY_UUID>(*dmu_category.uuid)), std::make_pair(NameOrIndex(NameOrIndex::NAME, DMU_SET_MEMBER_FK_DMU_CATEGORY_UUID),
...

	{
		...
		while ((step_result = sqlite3_step(stmt)) == SQLITE_ROW)
		{
			...
			if (uuid && fk_DMU_uuid)
			{
				...
				identifiers_map[*dmu_category.uuid].push_back(WidgetInstanceIdentifier(uuid, input_model_.t_dmu_category.getIdentifier(fk_DMU_uuid), code_string.c_str(), longhand_string.c_str(),
						0, flags_string.c_str(), TIME_GRANULARITY__NONE, MakeNotes(notes_string_1.c_str(), notes_string_2.c_str(), notes_string_3.c_str())));
			}
		}
