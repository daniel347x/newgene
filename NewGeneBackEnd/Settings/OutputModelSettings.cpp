...
SettingInfo OutputModelSetting::GetSettingInfoFromEnum(Messager & messager, int const value__)
{
...
void OutputModelSettings::SetMapEntry(Messager & messager, SettingInfo & setting_info, boost::property_tree::ptree & pt)
{
	...
	default:
		{
			boost::format msg("Unknown model output project setting \"%1%\" (\"%2%\") being loaded.");
...
OutputModelSetting * OutputModelSettings::CloneSetting(Messager & messager, OutputModelSetting * current_setting, SettingInfo & setting_info) const
{
	...
	try
	{
		...
		default:
			{
				boost::format msg("Unknown model output project setting \"%1%\" (\"%2%\") being requested.");
				msg % setting_info.text % setting_info.setting_class;
				messager.AppendMessage(new MessagerWarningMessage(MESSAGER_MESSAGE__INVALID_SETTING_INFO_OBJECT, msg.str()));
			}
			break;
...
OutputModelSetting * OutputModelSettings::NewSetting(Messager & messager, SettingInfo & setting_info, void const * setting_value_void)
{
	...
	default:
		{
			boost::format msg("Unknown model output project setting \"%1%\" (\"%2%\") being updated.");
			msg % setting_info.text % setting_info.setting_class;
			messager.AppendMessage(new MessagerWarningMessage(MESSAGER_MESSAGE__INVALID_SETTING_INFO_OBJECT, msg.str()));
		}
		break;
