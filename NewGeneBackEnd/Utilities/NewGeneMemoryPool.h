	...
	public:
		...
		void deallocate(char * const ptr, size_t n)
		{
			...
			if ((ptr >= previous_block_holding_deleted_item) && (ptr - previous_block_holding_deleted_item) / mySize < BLOCK_ITEM_COUNT)
			{
				...
				blockbits[previous_block_holding_deleted_item_index * BYTEBITS_PER_BLOCK + previous_index_to_deleted_item / 8] &= ~(static_cast<char>(0x01 <<
						(previous_index_to_deleted_item % 8)));
