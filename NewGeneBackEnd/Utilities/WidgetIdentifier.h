...
class WidgetInstanceIdentifier
{
	...
	public:
		...
		WidgetInstanceIdentifier(NewGeneUUID const uuid_, std::string const code_, std::string const description_, int const sequence_number_or_count_, char const * const flags_ = "",
								 TIME_GRANULARITY time_granularity_ = TIME_GRANULARITY__NONE, Notes notes_ = Notes())
		...
		WidgetInstanceIdentifier(NewGeneUUID const uuid_, WidgetInstanceIdentifier const & identifier_parent_, std::string const code_, std::string const description_,
								 int const sequence_number_or_count_, char const * const flags_ = "", TIME_GRANULARITY = TIME_GRANULARITY__NONE, Notes notes_ = Notes())
			: uuid(std::make_shared<NewGeneUUID>(uuid_))
		...
		WidgetInstanceIdentifier(std::string code_, std::string description_ = "", int const sequence_number_or_count_ = -1, char const * const flags_ = "",
								 TIME_GRANULARITY const time_granularity_ = TIME_GRANULARITY__NONE, Notes notes_ = Notes())
			: code(std::make_shared<std::string>(code_))
			, longhand(std::make_shared<std::string>(description_))
			, sequence_number_or_count(sequence_number_or_count_)
			, flags(flags_ ? flags_ : "")
		...
		identifier_parent; // Parent in the data model (i.e., foreign key in the case of a simple parent-child data relationship), not parent widget in the user interface
